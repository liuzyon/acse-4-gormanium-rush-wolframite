...
double Evaluate_Circuit(int* circuit_vector, int num_units,
    ...
    // Give an initial guess for the feed rate of both
    // components to every cell in the circuit
    for (int i = 0; i < num_units; i++) {
        units[i].init();
    }
    ...
    while (iter_num < max_iterations) {
        // For each unit use the current guess of the feed
        // flowrates to calculate the flowrate of each component in both the
        // concentrate and tailings streams
        for (int i = 0; i < num_units; i++) {
            units[i].cal_con_tail();
        }

        // Store the current value of the feed to each cell as
        // an old feed value and set the current value for
        // all components to zero
        for (int i = 0; i < num_units; i++) {
            units[i].reset();
        }

        // Set the feed to the cell receiving the circuit feed equal to
        // the flowrates of the circuit feed
        units[circuit_vector[0]].init();

        // Go over each unit and add the concentrate and tailings flows
        // to the appropriate unit’s feed
        // based on the linkages in the circuit vector. This will also
        // result in an updated estimate for the
        ...
        steady_unit = 0;
        // Check the difference between the newly calculated feed rate
        // and the old feed rate for each cell.
        // If any of them have a relative change that is above a given
        // threshold then repeat from step 2; otherwise, leave the loop.
        for (int i = 0; i < num_units; i++) {
    ...
    //  Based on the flowrates of the overall circuit concentrate stream,
    //  calculate a performance value for the circuit.
    ...
    if (steady_unit == num_units) {
        ...
        // If there is no convergence, use the worst possible
        // performance as the performance value
        // the flowrate of waste in the feed times the value of
...
#pragma omp critical
                ...
                double score_child1 = Evaluate_Circuit(child_1.curit_vector,
                    num_units, 1e-6, 1000);
                double score_child2 = Evaluate_Circuit(child_2.curit_vector,
                    num_units, 1e-6, 1000);
...
#pragma omp critical
    ...
    std::cout << "Best fitness didn't change in 500 generation, "
        "quit the Genetic algorithm" << std::endl;
